...
const page = () => {
  ...
  useEffect(() => {
    ...
    reader.onload = (event) =>
      setconfigs({ ...configs, file: event.target.result }); // desired file content
  ...
  const saveConn = (e) => {
    ...
    const configData = {
      ...
      ssh: {
        ...
        port: port,
        username:
          host && port && username && sshkey && sshfilename && sshcommand
            ? username
            : "",
        privateKey:
          host && port && username && sshkey && sshfilename && sshcommand
  ...
  return (
    <Suspense fallback={<Loader/>}>
      <div>
        <h1 className="text-center my-[2rem] text-[25px]">
          Create New Connection
        </h1>
        <form
          className="flex flex-col max-w-[40rem] w-[90vw] mx-auto"
          onSubmit={saveConn}
        >
          <label className="flex flex-col">
            ...
            <input
              ...
              value={configs.name}
              type="text"
              className={`bg-[#11232b] text-slate-200 text-lg p-[10px] rounded-lg ${
                configs.name ? "border outline-green-400 border-green-400 " : ""
              }`}
            />
          </label>
          <br />
          ...
          <div className="flex w-full  gap-4 max-sm:flex-col">
            <label className="flex flex-col w-full">
              Pass Key
              <div
                className={`relative flex flex-row items-center rounded-lg border ${
                  false
                    ? "outline-green-400 border-green-400 "
                    : "border-[#11232b]"
                }`}
              >
                <input
                  ...
                  value={passkey}
                  type="password"
                  className={`bg-[#11232b] text-slate-200 text-lg p-[10px] rounded-lg w-full`}
                />
                <p
                  onClick={() => {
                    hideshow("password");
                  }}
                  className="bg-[#11232b] border-[2.5px] border-[#11232b] p-[10px] rounded-lg absolute right-0 cursor-pointer"
                >
                  <img
                    id="passwordtoggle"
          ...
          <details open className="rounded-lg p-5 border border-[#11232b]">
            ...
            <label className="flex flex-col relative">
              ...
              <div
                className={`${
                  configIndentation == 1
                    ? "border outline-green-400 border-green-400"
                    : configIndentation == 2
                    ? "border outline-red-400 border-red-400"
                    : ""
                }`}
              >
                <Editor
              ...
              <div className="bg-blue-500 h-[64px] w-[64px] rounded-[50%] flex items-center justify-center absolute bottom-5 right-5 cursor-pointer hover:bg-blue-600 hover:shadow-md">
                <input
                  ...
                  type="file"
                  className="bg-transparent h-[64px] w-[64px] absolute opacity-0 cursor-pointer"
                />
                <svg
                  version="1.0"
                  width="32.000000pt"
            ...
            <label className="flex flex-col">
              ...
              <input
                ...
                readOnly
                type="text"
                className={`bg-[#11232b] text-slate-200 text-lg p-[10px] rounded-lg outline-none ${
                  url ? "border border-green-400 " : ""
                }`}
              />
            </label>
          </details>
          ...
          <details className="rounded-lg p-5 border border-[#11232b]">
            ...
            <label className="flex flex-col">
              ...
              <div
                className={`bg-[#11232b] text-slate-200 text-lg p-[10px] rounded-lg ${
                  false ? "border outline-green-400 border-green-400 " : ""
                }`}
              >
                <input
                  onChange={(e) => {
                    setsshfile(e.target.files[0]);
                  }}
                  type="file"
                  className="opacity-0 absolute"
                />
                <div className="bg-[#11232b] text-[#5bb0cd] z-1">
                  {" "}
                  {sshfilename || "Add ssh private key file"}{" "}
                </div>
              </div>
            </label>
            <br />
          ...
          <div className="flex max-lg:flex-wrap w-[100%] gap-[1rem] mb-[5rem]">
            <div
              ...
              className={`w-full border ${
                k8sconnchecked == 1 || sshconnchecked == 1
                  ? "border-[#38ad47]"
                  : k8sconnchecked == 2
                  ? "border-[#a93c2b]"
                  : "border-[#061924]"
              } bg-[#061924] p-[16px] rounded-lg mt-[2rem] flex items-center gap-5 justify-center min-w-[16rem] cursor-pointer`}
            >
              {(k8sconnchecked === 0) & (sshconnchecked === 0) ? (
                <p>Check Connection</p>
              ) : (
                ""
              )}
              {(k8sconnchecked === 4) & (sshconnchecked === 4) ? (
                <p>Backend refused to connect</p>
              ) : (
                ""
              )}
              {(k8sconnchecked === 5) & (sshconnchecked === 5) ? (
                <p>Empty backend URI</p>
              ) : (
                ""
              )}
              {k8sconnchecked == 1 ? (
            ...
            <button
              type="submit"
              value="Save"
              className="w-full border border-[#061924] bg-[#061924] p-[16px] rounded-lg md:mt-[2rem]"
            >
              Save & Connect
            </button>
          </div>
